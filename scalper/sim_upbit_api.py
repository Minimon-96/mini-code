...
class simul:
    def __init__(self):
        ...
        self.cur_cash = 10000000
        self.cur_balance = 0
        self.avg_buy_price = 0
        self.order_info = 0
        self.order_state = 0

        self.get_cur_price_idx = 0

    # @log_function_call
    # def get_price(self, chk_next=0):
    #     # JSON 파일 경로
    #     #logPath="D:\\mygit\\coin\\coin_study\\tb\\trace" # window
    #     file_path = "D:\\mygit\\coin\\coin_study\\Price_Data\\upbit_data\\ripple_20230802.json"

    #     # JSON 파일 열기
    #     with open(file_path, "r") as file:
    #         data = json.load(file)

    #     res = data[self.get_cur_price_idx][2]
    #     log("TR","Success",res)
    #     if int(chk_next) == 1:
    #         self.get_cur_price_idx += 1
    ...
    def GET_MARKET_TREND(self, ticker, price, days_short, days_long):  
        ...
        try:
            ...
            log("TR", "Cur Price:"+str(price), "Trend price:"+str(round(last_ma_short)),"Trend:"+trend)
            return trend
        except Exception as e:
    ...
    def GET_ORDER_STATE(self, uuid):   # 주문 상태 리턴 (오류:error / 대기:wait / 완료:done)
        try:
            ...
            #cur_price = simul.get_price(chk_next=1)
    ...
    def sim_Buy_order(self, ticker, amount):
        ...
        if re.match(r'^[A-Z]{3}-[A-Z]{3}$', ticker):
            ...
            chk_ticker = 0
            #log("TR","Fail","Ticker error")

        # amount 값 확인
        if isinstance(amount, int) and amount >= 5000:
            chk_amount = 1
        else:
            log("TR","Fail","Amount error")
            chk_amount = 0
        if chk_ticker == 0 or chk_amount ==  0:
            fail_reason = {'error' : "Check the Parameter",
                           'ticker': "KRW-XRP",
                           'amount' : "5000 up"}
            log("TR","Fail","Check the param")
            return fail_reason
        
        try:    
            self.uuid = uuid.uuid4()
            self.price = GET_CUR_PRICE(coin)
            #self.price = simul.get_price(self, chk_next=0)
            ...
            res = {  'uuid' : self.uuid, 
                        ...
                        'volume' : self.volume}
            #log("TR","Success", res)
            # 매수진행시 평균매수단가 계산 (현재 보유 갯수/구매가격)
            # (N차 매수가격 * 수량 + (N-1)차 까지의 매수금액) / 전체수량
            self.cur_balance += self.volume
            self.cur_cash -= amount
            self.avg_buy_price = (self.price * self.volume + temp_amount) / self.cur_balance
        except Exception as e:
            log("TR", "FAIL", e)
            res = 0
        return res
    ...
    #@log_function_call
    def sim_Sell_order(self, ticker):
        #log("TR","SELL ORDER TICKER : "+ticker)
        #  request : ticker, volume
        #       ex : KRW-XRP, 100
        # response : ask, price, volume
        #       ex : ask, 600, 100
        try:    
            volume = simul.GET_QUAN_COIN(self, ticker)
            if volume > self.cur_balance:
                ...
                price = GET_CUR_PRICE(coin)
                #price = simul.get_price(self, chk_next=0)
    ...
    @log_function_call
    def sim_limit_Sell_order(self, ticker):
        #  request : ticker, buy_avg_price, volume
        #       ex : KRW-XRP, 612, 100
        # response : uuid, ask, price, volume
        #       ex : last_order_uuid, ask, 612, 100
        #buy_avg_price = simul.GET_BUY_AVG(ticker)
        buy_avg_price = round(self.avg_buy_price * 1.01)
        #volume = last_order_volume
        if self.last_order_price == "0":
            self.last_order_uuid = uuid.uuid4()
        self.last_order_price = buy_avg_price
        self.last_order_state = 'ask'
        self.last_order_volume = self.cur_balance

        if self.last_order_volume > self.cur_balance:
            res = {
                'state' : "error",
                'msg' : "Please check the quantity to be sold."
            }
