...
def log(level, *args):
    ...
    if level not in ("TR", "DG"):   # 로그레벨 미지정시 에러
...
@log_function_call
def ORDER_SELL_LIMIT(ticker, *args):    # 지정가 매도 주문 결과 리턴 (지정한 Minimum Cash 가격에 도달한 경우 진행)
    try:
        vol = math.floor(upbit.get_balance(ticker))     # 매도 수량 지정(소수점 첫째 자리에서 내림계산)
        buy_avg_price = math.floor(1.02 * GET_BUY_AVG(ticker))      # 평균 매수가를 매도 주문 가격으로 지정
        res = upbit.sell_limit_order(ticker, buy_avg_price, vol)    
        if 'error' in res:
            log("TR","Error", ticker, buy_avg_price, res)
            res = 0
            return res
        log("TR", " Success", ticker, buy_avg_price,res)
        res = 1
    except Exception as e:
...
def run(chk_run):
    ...
    while chk_run == 1:
        ...
        min_cash = round((cur_cash + (cur_coin * cur_price)) * Last_order_limit/100)  # 최초 금액 대비 (10%) 가 되면 매수중지
            ...
            try:
                ...
                if cur_price < buy_price:   # 가격이 지정한 매수가 밑으로 내려오면 매수 진행
                    ...
                    if res != 0:
                        ...
                        log("DG","Check 15 Minute Timer : " + str(chk_15m_timer))
        ...
        while chk_sell_order == 1:
            try:
                ...
                if order_status == 'done':
                    log("DG", "Sell Order Status : " + order_status)
                    chk_run = 0 # 매도 주문이 완료되면 다시 메인 코드 진행
                    chk_sell_order = 0
                else:
                    log("DG", "Sell Order Status : " + order_status)
                    chk_run = 0
